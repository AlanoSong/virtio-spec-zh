...
### 2.2.3 旧版接口：特性位注意事项
- 过渡性驱动**必须**通过侦测 VIRTIO_F_VERSION_1 特性位未被提供，来识别旧版设备。过渡性设备**必须**通过侦测驱动未确认 VIRTIO_F_VERSION_1 来识别旧版驱动。
- 在这种情况下，[驱动] 将通过旧版接口来调用设备。
- 旧版接口支持是**可选**的。因此，过渡性和非过渡性的设备和驱动都遵守本规范。
- 过渡性设备和驱动相关的要求，包含在名为“传统接口”的章节中，就像这一节一样。
- 当通过旧版接口使用设备时，过渡性设备和驱动**必须**按照这些旧版接口章节中写明的要求运行。这些章节内的规范文本通常不适用于非过渡型设备。
