...
## 2.6 虚拟队列
- 在虚拟机设备上进行大量数据传输的方法，叫做虚拟队列。每个设备可以拥有 0 个或多个虚拟队列。
- 虚拟 I/O 设备最多可以有 65536 个虚拟队列。每个虚拟队列都通过一个虚拟队列索引来标识。虚拟队列索引的值在 0 到 65535 之间。
...
- 2. 可用环形缓冲区 - 等同于`驱动区域`
- 3. 已使用环形缓冲区 - 等同于`设备区域`
- 以下 2 种格式可供选择：`分散式虚拟队列`（参考 2.7 分散式虚拟队列）和`整合式虚拟队列`（参考 2.8 整合式虚拟队列）。
- 任一驱动和设备要么支持整合式虚拟队列格式，要么支持分散式虚拟队列格式，或者两者都支持。
...
## 2.7 分散式虚拟队列
- 分散式虚拟队列格式，是本协议 1.0 版本（和更老版本）支持的唯一格式。
- 分散式虚拟队列格式，将虚拟队列拆分成多个部分，其中每一部分均可由驱动或设备中的任意一方进行写入操作，但但不能同时由两者进行操作。在标记缓冲区可用，以及标记缓冲区已使用时，需要更新多个[分散式虚拟队列的]部分，（以及/或者）部分内部的某些位置。
- 每个队列拥有 16 位的队列大小参数，该参数设置[队列的]入口数量，并且表明队列的整体大小。
- 每个队列包含以下 3 个部分：
- 1. 描述符表 - 存在于`描述符区域`
- 2. 可用环形缓冲区 - 存在于`驱动区域`
- 3. 已使用环形缓冲区 - 存在于`设备区域`
- 其中，每个部分在客户机内存中都是物理连续的，并且有不同的内存对齐要求。
- 内存对齐和大小要求，是用字节数来表示的，虚拟队列的每个部分的要求总结在下表中：
...
- `对齐`这一列，给出了虚拟队列每个部分的最小对齐要求。
- `大小`这一列，给出了虚拟队列每个部分总的字节数。
- `队列大小`对应虚拟队列缓冲区的最大数量。`队列大小`的值总是 2^n。`队列大小`的最大值是 32768。
- 当驱动想要向设备发送一个数据缓冲区时，它会在`描述符表`中的一个位置填入数据（或者将多个描述符串联起来），并将描述符索引写入`可用环形缓冲区`中。然后驱动会通知设备。当设备完成一个数据缓冲区的处理后，它会将描述符索引写入`已使用环形缓冲区`中，并发送一个`已使用缓冲区通知`。

### 2.7.1 驱动要求：虚拟队列
- 驱动***必须***确保，每个虚拟队列部分的第一个字节的物理地址，必须是上述表格中指定对齐值的倍数。

### 2.7.2 旧版接口：虚拟队列布局注意事项
- 对于旧版接口，对虚拟队列的布局还设置了若干额外的限制：
- 每个虚拟队列占用 2 个或更多物理上连续的页面（通常定义为 4096 字节，但具体取决于传输方式；此后统称为`队列对齐`），并由三个部分组成：

- | 描述符表 | 可用环形缓冲区（...填充...） | 已使用环形缓冲区 |
- 总线相关的`队列大小`字段，控制着虚拟队列的总字节数。在使用旧版接口时，过渡驱动***必须***从设备中获取`队列大小`字段，并且***必须***根据以下公式（qalign 给定`队列对齐`，qsz 给定`队列大小`）为虚拟队列分配总字节数：
```c
...
```
- 由于字节填充的原因，会浪费一些内存空间。当使用旧版接口时，过渡性设备和驱动都***必须***使用以下的虚拟队列结构体，来放置虚拟队列的成员：
```c
struct virtq {
...
### 2.7.3 旧版接口：虚拟队列字节序注意事项
- 需要注意的是，当使用旧版接口时，过渡性的设备和驱动***必须***使用客户机原生的字节序，作为各个[特性]字段和虚拟队列的字节序。这与本协议所规定的，非旧版接口的小端字节序相反。默认主机已经知晓客户机的字节序。
...
- 请注意，一些设备实现，对描述符的总大小有较大的但合理的限制（例如基于主机 OS 的 IOV_MAX）。但在实际应用中，这并不是一个问题：不会对创建不合理大小描述符（例如将网络数据包分割成 1500 个单字节描述符）的驱动给予太多侧重。
...
#### 2.7.4.3 旧版接口：消息帧封装
- 遗憾的是，最初的驱动实现采用了简单的布局方式，而设备也依赖于这种布局方式，尽管本规范中并未明确说明这一点。此外，virtio_blk SCSI 命令的规范，要求从帧边界来推断字段长度（见 5.2.6.3 旧接口：设备操作）。
- 因此，在使用旧版接口时，VIRTIO_F_ANY_LAYOUT 特性向设备和驱动表明，对于帧结构没有做出任何假设。当未进行此协商时，过渡性驱动的要求包含在每个设备部分中。

### 2.7.5 虚拟队列描述符表
- 描述符表指的是驱动为设备所使用的缓冲区。`addr`是一个物理地址，这些缓冲区可以通过`next`进行链接。每个描述符描述一个缓冲区，对于设备而言，该缓冲区是只读的（“设备可读”）或只写入的（“设备可写”），但缓冲区链表可以包含，设备既可读又可写的缓冲区。
...
```
- [描述符]表中描述符的数量，是通过该虚拟队列的队列大小来定义的：这是描述符链表长度可能的最大值。
- 如果[驱动和设备]协商过 VIRTIO_F_IN_ORDER 特性，驱动在环形缓冲区中使用描述符时遵循：从表的 0 偏移处起始，在表的尾部再折返到头部。
- ***注意***：旧版协议[Virtio PCI Draft]将此结构称为`vring_desc`，并将相关常量称为 VRING_DESC_F_NEXT 等，但其布局和值是完全相同的。
