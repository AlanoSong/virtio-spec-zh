...
- 已使用环形缓冲区，是设备在处理完缓冲区后，返回这些缓冲区的地方：该缓冲区仅由设备写入，并由驱动读取。
- 环形缓冲区中的每个条目都是一个对：*id* 表示描述符链的头部条目，该链表描述了缓冲区（这与客户机之前在可用环形缓冲区中放置的条目相匹配），而 *len* 是写入到缓冲区中的字节数的总和。
- **注意**：*len* 对于使用非信任缓冲区的驱动特别有用：如果驱动不知道设备实际写入了多少数据，那么驱动就必须提前将缓冲区清零，以确保不会发生数据泄露。
- > 例如，网络驱动可能会直接将接收到的缓冲区，传递给无特权用户空间应用程序。如果网络设备没有覆盖该缓冲区中的字节，这可能会将其他进程中释放的内存内容泄露给应用程序。
- *idx* 字段指示设备将在环形缓冲区中何处放置下一个描述符条目（取模于队列大小）。它从 0 开始，并递增。
- **注意**：传统 [协议] 将这些结构体描述位 vring_used 和 vring_used_elem，并且将该常量 [上文中的 VIRTQ_USED_F_NO_NOTIFY] 描述为 VRING_USED_F_NO_NOTIFY，但是布局和值是相同的。

#### 2.7.8.1 传统接口：virtqueue 已使用环形缓冲区
- 历史上，许多驱动都忽略了 *len* 值，因此许多设备的 *len* 值设置不正确。所以，在使用传统接口时，如果可能的话，在使用的环形缓冲区条目中忽略 *len* 值通常是一个好做法。针对每种设备类型，都列出了具体的已知问题。

#### 2.7.8.2 设备要求：virtqueue 已使用环形缓冲区
- 设备在更新已使用 *idx* 之前，**必须**先设定 *len*。
- 设备在更新已使用 *idx* 之前，**必须**从第一个设备可写缓冲区开始，向描述符写入至少 *len* 个字节。
- 设备**可以**向描述符写入多于 *len* 个字节的数据。
- **注意**：可能存在一些错误情况，即设备可能不知道哪些缓冲区的数据已被写入。这就是为什么允许 *len* 作为低估值的原因：这比驱动认为未初始化的内存已被覆盖要好，因为实际上并未发生这种情况。

#### 2.7.8.3 驱动要求：virtqueue 已使用环形缓冲区
- 驱动**禁止**对于设备可写缓冲区 *len* 字节长度以外的数据，做任何假设，并且**应该**忽略这些数据。
...
### 2.7.10 可使用缓冲区通知抑制
- 设备能够以类似于驱动在第 2.7.7 节中所描述的方式，抑制可用缓冲区通知。设备对使用环形缓冲区中的 *flags* 或 *avail_event* 进行操作的方式，与驱动对可用环形缓冲区中的 *flags* 或 *used_event* 进行操作的方式相同。

#### 2.7.10.1 驱动要求：可用缓冲区通知抑制
- 在分配已使用环形缓冲区时，驱动**必须**将已使用环形缓冲区中的 *flags* 设为 0。
- 如果未协商 VIRTIO_F_EVENT_IDX 特性位：
- 1. 驱动**必须**忽略 *avail_event* 的值。
- 2. 在驱动将描述符索引写进可用环形缓冲区后：
- 2.1 如果 *flags* 是 1，驱动**不应该**发送通知。
- 2.2 如果 *flags* 是 0，驱动**必须**发送通知。
- 否则，如果协商了 VIRTIO_F_EVENT_IDX 特性位：
- 1. 驱动**必须**忽略 *flags* 的低比特位。
- 2. 在驱动将描述符索引写进可用环形缓冲区后：
- 2.1 如果可用环形缓冲区中的 *idx* 字段等于 *avail_event*，驱动**必须**发送通知。
- 2.2 否则驱动**不应该**发送通知。

#### 2.7.10.2 设备要求：可用缓冲区通知抑制
- 如果未协商 VIRTIO_F_EVENT_IDX 特性位：
- 1. 设备**必须**将 *flags* 设为 0 或 1.
- 2. 设备**可以**将 *flags* 设为 1，来建议驱动：不需要通知。
- 否则，如果协商过 VIRTIO_F_EVENT_IDX 特性位：
- 1. 设备必须将 *flags* 设为 0。
- 2. 设备**可以**使用 *avail_event* 来告知驱动，在驱动将具有由 *avail_event* 指定的索引的条目写入可用环形缓冲区之前（即在可用环形缓冲区的 *idx* 值达到 *avail_event* + 1 之前），无需进行通知操作。
- 设备**必须**处理来自驱动的虚假通知。

### 2.7.11 操作 virtqueue 的辅助工具
- Linux 内核源码中，在 include/uapi/linux/virtio_ring.h 中包含了上述定义，并且提供了更易于使用的辅助例程。这部分代码由 IBM 和 Red Hat 在 BSD 许可证下授权，因此可以在其他项目中自由使用，并且这些代码在 virtio_queue.h 中也重新实现了一份（做了细微改动）。

### 2.7.12 virtqueue 操作
- virtqueue 操作分为两块：[驱动]为设备提供新的可用缓冲区，以及[设备]处理已使用缓冲区。

- **注意**：举例来说，最简单的 virtio 网络设备包含 2 个 virtqueue：发送 virtqueue 和接收 virtqueue。驱动向发送 virtqueue 添加数据包（设备可读），并且在用完后清理掉这些数据包。同样的，接收到的数据包（设备可写）会被添加到接收 virtqueue 中，并且等待处理。
- 接下来详细描述，这 2 块操作如何使用分散式 virtqueue。

### 2.7.13 向设备提供缓冲区
- 驱动按照以下步骤，向设备的 virtqueue 提供缓冲区：
...
#### 2.7.13.1 将缓冲区添加到描述符表中
- 一个缓冲区包含 0 个或者多个设备可读且物理 [地址] 连续的元素，后面跟着 0 个或者多个设备可写且物理 [地址] 连续的元素（每个都包含至少一个元素）。以下算法将其对应到描述符表中，来形成一个描述符链表：
- 对于每个缓冲区元素（b）：
- 1. 获取下一个空闲描述符表条目（d）
- 2. 将 *d.addr* 值写到（b）的物理地址开始处
- 3. 将 *d.len* 设置成（b）的长度
- 4. 如果（b）是设备可写的，将 *d.flags* 设置成 VIRTQ_DESC_F_WRITE，否则设置成 0
- 5. 如果这之后还有一个缓冲区：
- 5.1 （a）将 *d.next* 设置成下一个可用的描述符元素的索引。
- 5.2 （b）在 *d.flags* 中设置 VIRTQ_DESC_F_NEXT 位。
- 实际上，*d.next* 通常用于连接空闲描述符，并且还会有一个单独的计数器，来在开始映射操作之前检查是否有足够的空闲描述符可用。

#### 2.7.13.2 更新可用环形缓冲区
- 描述符链表头是上述算法中的第一个（d），即指向缓冲区首部的描述符表条目的索引。一个简单的驱动实现**可能**会执行以下操作（假定已进行了适当的小端到大端和大端到小端的转换）：
```c
avail->ring[avail->idx % qsz] = head;
```
- 然而，通常情况下，驱动在更新 *idx* 之前**可能**会添加许多描述符链表（此时这些链表对设备而言就变得可见了），因此通常会保留一个计数器来记录驱动已添加的链表的数量：
```c
avail->ring[(avail->idx + added++) % qsz] = head;
```

#### 2.7.13.3 更新 *idx*
- *idx* 总是递增的，并且在达到 65536 时循环回 0：
```cpp
avail->idx += added;
```
- 一旦驱动更新了可用 *idx*，就会公开该描述符及其内容。设备**可以**立即访问驱动创建的描述符链表以及它们所引用的内存。

##### 2.7.13.3.1 驱动要求：更新 *idx*
- 驱动**必须**在 *idx* 更新之前实施一个合适的内存屏障，来确保设备看到最新的[内存]拷贝。

#### 2.7.13.4 通知设备
- 实际上的设备通知方法，是总线特定的，但是总体上这些方法比较耗时。所以，如果设备不需要它们，**可以**抑制这样的通知，章节 2.7.10 有详细描述。
- 在检查通知是否受到抑制之前，驱动必须小心处理新的 *idx* 值。

##### 2.7.13.4.1 驱动要求：通知设备
- 在读取 *flags* 或者 *avail_event* 之前，驱动**必须**实施一个合适的内存屏障，来避免漏掉某个通知。

### 2.7.14 从设备接收已使用缓冲区

- 一旦设备处理完描述符（读或者写它们，或者单一操作，取决于 virtqueue 和设备）所引用的缓冲区数据后，设备会按照章节 2.7.7 中描述的方式，发送一个已使用缓冲区通知。
- 注意：出于性能考虑，处理已使用环形缓冲区时，驱动**可以**禁用已使用缓冲区通知，但同时要注意，在清空环形缓冲区和重新使能通知的间隙，可能会出现通知丢失的问题。这个问题可以通过，在重新启用通知后，重新检查已使用缓冲区的方式来解决：
